...
namespace MusicCatalogue.Api.Controllers
{
    ...
    public class RetailersController : Controller
    {
        ...
        {
            _factory.Logger.LogMessage(Severity.Debug, $"Retrieving list of retailers");
        ...
        {
            _factory.Logger.LogMessage(Severity.Debug, $"Retrieving retailer with ID {id}");

            var retailer = await _factory.Retailers.GetAsync(x => x.Id == id);

            if (retailer == null)
            {
                _factory.Logger.LogMessage(Severity.Error, $"Retailer with ID {id} not found");
                return NotFound();
            }

            _factory.Logger.LogMessage(Severity.Debug, $"Retrieved retailer {retailer}");
            return retailer;
        }
        ...
        {
            _factory.Logger.LogMessage(Severity.Debug, $"Adding retailer {template}");
        ...
        {
            _factory.Logger.LogMessage(Severity.Debug, $"Updating retailer {template}");
        ...
        {
            _factory.Logger.LogMessage(Severity.Debug, $"Deleting retailer with ID {id}");
            ...
            catch (RetailerInUseException)
            {
                ...
                _factory.Logger.LogMessage(Severity.Error, $"Retailer with ID {id} has purchases associated with it and cannot be deleted");
                return BadRequest();
            }
